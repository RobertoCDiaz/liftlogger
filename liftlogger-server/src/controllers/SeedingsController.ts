...
export default class SeedingsController {
  ...
  async defaultUserSeeding(userEmail: string) {
    const muscleGroups: MuscleGroupCreationParams[] = getMuscleGroupsFixture()
  ...
  async seedTestData() {
    ...
    this.prisma
      .$transaction([
        ...
        this.prisma.movementNote.createMany({ data: getMovementNotesFixture() }),
  ...
  async seedFakeData(userEmail: string) {
    this.prisma.$transaction(async () => {
      ...
      const userGroups = await this.prisma.muscleGroup.findMany({
      ...
      console.log('movIds: ', movementsIds);
